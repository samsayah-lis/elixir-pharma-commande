            ...
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <div style={{ padding:"20px 24px", borderBottom:"1px solid #f0f2f5", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div>
                  ...
                  <div style={{ fontSize:12, color:"#888", marginTop:2 }}>{pharmacyName} · {myOrders.length} commande(s)</div>
                </div>
              ...
              <div style={{ flex:1, overflowY:"auto", padding:"16px 24px" }}>
                {myOrdersLoading && <div style={{ textAlign:"center", padding:40, color:"#888" }}>Chargement...</div>}
                ...
                {myOrders.map(order => {
                  ...
                  return (
                    <div key={order.id} style={{ background:"#fafbfc", borderRadius:14, padding:"16px 20px", marginBottom:12, border:"1px solid #e8ecf0" }}>
                      ...
                      <button onClick={() => {
                        items.forEach(it => {
                          if (!it.cip || !it.qty) return;
                          Object.entries(CATALOG_WITH_ADMIN).forEach(([catKey, c]) => {
                            const idx = (c.products || []).findIndex(p => p.cip === it.cip);
                            if (idx >= 0) setQuantities(prev => ({ ...prev, [`${catKey}-${idx}`]: parseInt(it.qty) || 0 }));
                          });
                        });
                        setShowMyOrders(false);
                        flash(`✓ ${items.length} produit(s) ajoutés au panier`);
                      }} style={{
